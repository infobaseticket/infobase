...
*/
	...
	if (query.module){
		...
		if(query.module=='RAF' && query.rafid){
			...
			var load='scripts/rafreport/rafreporttable.php?module='+query.module+'&report='+query.report+'&region='+query.region;	
		...
		$('#maincontent').load(load, function(){
			...
			if (query.module && query.module!='rafreport'){
				$("#searchForm").ajaxSubmit(options); 
					...
					if (query.report=='RFBCS'){
					...
					}else if(query.report=='RFFUNDBLOCKED'){
						var report='Base RF - FUNDING BLOCKED';
					}else if(query.report=='RFPAC'){
						var report='Base RF - PAC';
					}else if(query.report=='RFBCSA15'){
						var report='Base RF - BCS NET1';
						...
						var report='Base Delivery - ACQ PARTNER';
					...
					}else if(query.report=='DELCON'){
						var report='Base Delivery - CON PARTNER';
					}else if(query.report=='DELCOFACQ'){
						var report='Base Delivery - COF ACQ';
					}else if(query.report=='DELCOFCONPM'){
						var report='Base Delivery - COF CON PM';
					}else if(query.report=='DELCOFCONTS'){
						var report='Base Delivery - COF CON TS';
					
					}else if(query.report=='PARTNERCOFACQ'){
						var report='Partner - COF ACQ';
					}else if(query.report=='PARTNERINP'){
						var report='Partner - INPUT';
					}else if(query.report=='PARTNERA304'){
						var report='Partner - A304';
					}else if(query.report=='PARTNERLBP'){
						var report='Partner - LBP';
					}else if(query.report=='PARTNERACQ'){
						var report='Partner - ACQUIRED';
					}else if(query.report=='PARTNERCOFCON'){
						var report='Partner - COF CON';
					}else if(query.report=='PARTNERRFPACK'){
						var report='Partner - SUBMIT RF PACK';
					}else{
						alert('report not defined');
					}
					*/
					$('#allocated').val(query.partner);
	...
	*/
	...
	$("#mainnavbar").on("click",".mainnav",function( e ){
		...
		$("#maincontent").html('<span class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></span> Loading...');
	...
	$("#maincontent").on("submit","#searchForm",function( e ){
		$("#siteTabs li").remove();
	...
	$("#maincontent").on("click","#searchbutton",function( e ){
		$("#siteTabs li").remove();
		$("#contentTabs div").remove();
		$("#rafActionsOverview").hide('fast');
		//$("#search_output").hide('fast');
...
* RAFACTIONSOVERVIEW
*****************************************************************************************/
	$("body").on("click",".rafctionsNav",function( e ){
		e.preventDefault();	
		var partner=$(this).data('partner');
		$('.rafctionsNav').removeClass('active');
		$(this).addClass('active');
	  	$.ajax({
			type: "POST",
			...
			success : function(data){
			    $('#rafActionsOverviewOutput').hide().html(data).fadeIn(1000); 
			    $('#spinner').spin(false);
			},
			beforeSend: function ( xhr ) {
				$('#spinner').spin();
...
*****************************************************************************************/
	$("body").on("click",".rafnav",function( e ){
		...
		if (action=="view"){
			...
			}else if (status.indexOf("BASE PM/TS")=="0"){
				viewfile='raf_details_cof';
			}else{
...
$("body").on("click","#genrateCOF",function(e){
	...
	$('#spinner').spin("large");
	bootbox.dialog({
        message: 'Are you sure you want to genratea COF file?',
        title: 'GENRATE COF',
        buttons: {
            success: {
                label:"I'm sure",

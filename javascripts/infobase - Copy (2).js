...
*****************************************************************************************/
	...
	$("body").on("click",".bsdsdetails2",function( e ){
		...
		if (clicked_tab==="DELETEBSDS"){
			...
			bootbox.dialog({
	            message: 'Click DELETE if you want to DELETE the BSDS with key "'+key,
	            title: 'FREEZE BSDS',
	            buttons: {
	                success: {
	                    label:"DELETE",
	    	...
	    	var formData= {
				...
				'candidate': 		candidate,
				'upgnr': 			upgnr,
				'siteid': 			$('#bsdsform_'+id+' input[name="siteid"]').val(),
		...
		}else if (clicked_tab!=="CHANGEID" && clicked_tab!=="DELETE" && clicked_tab!=="REMOVEFUNDING" && clicked_tab!=="FREEZEBSDS"){ 			
			...
			if (clicked_tab==="PRINT"){
				...
				$('#siteTabs').addCtab('siteTabs',targettype,'glyphicon-book',title);
				$('#'+targettype).empty();

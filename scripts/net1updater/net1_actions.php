...
function get_net1_date($siteid,$candidate,$code,$upgnr,$type,$conn_Netone){
	global $config;
	...
	if($type=="UPG"){
		...
		(
		 	WOT.WOT_TOS_TAS_CODE='".$code."'
			AND SIT.SIT_UDK='".$candidate."'
			AND WOT_SIT_ID IS NOT NULL
			AND WOT.WOT_SIT_ID=SIT.SIT_ID
			AND WOT_SIT_UDK='".$candidate."'
			AND WOT_WOR_UDK='".$siteid."'
		)or (
			WOT.WOT_TOS_TAS_CODE='".$code."'
			AND SIT.SIT_UDK='".$candidate."'
			AND WOT_WOR_UDK='".$siteid."'
...
function updatePartie($site_upgnr,$code,$partnerCode){
	...
	if (substr($site_upgnr,0,2)=='99' || $code=='WIP-A' || $code=='SAC' || $code=='ALSAC'){ //UPG
	...
	if ($amount==0){
		if (substr($site_upgnr,0,2)=='99' || $code=='WIP-A' || $code=='SAC' || $code=='ALSAC'){
		...
		'".strtoupper(trim($partnerCode))."','".strtoupper(trim($code))."' ,'FEYLAND',SYSDATE)";
		//echo $query."<hr>";
		...
		$query="UPDATE TRANSACTION_PARTIES@".$config['net1db']." SET TXP_PTY_ID='".strtoupper(trim($partnerCode))."', TXP_LAST_USER= 'FEYLAND',
...
function updatecode($site,$candidate,$upgnr,$code,$insertdate,$estimate,$notes,$override,$conn_Netone,$filename){
	global $conn_Infobase;
	global $guard_username;
	global $keymilestones;
	global $config;
	...
	$net1_date=get_net1_date($site,$candidate,$code,$upgnr,$type,$conn_Netone);
		...
		if ($net1Already=='NOT SET' or $override=="yes"){
			if (($start=="A" && $upgnr=='') or ($start=="U"  && $upgnr!='')){
				...
				if (!$stmt){
					...
					$type="info";
					$query2="INSERT INTO NET1UPDATER_LOG VALUES (SYSDATE,'".$site."','".$upgnr."','".$code."', '".$guard_username."','".$insertdate."','".$estimate."','".$notes."','".$filename."')";
...
if ($_POST['action']=="updatecode"){
	...
	if ($_POST['element']!=""){
		$epl=explode(":",$_POST['element']);
		$siteid=trim($epl[0]);
		$candidate=trim($epl[1]);
		
		if (strlen($siteid)==7 && strlen($candidate)==8){
			$code=explode("-",$_POST['code']);
			$code=$code[0];
			$notes=escape_sq($_POST['notes']);
			$out=updatecode($siteid,$candidate,$_POST['upgnr'],$code,$_POST['insertdate'],$_POST['estimate'],$notes,$_POST['override'],$conn_Netone,$filename);
			if ($out=="error"){
	...
	echo json_encode($res);
		...
		if ($start=="A" && $_POST['upgnr']==''){
			if ($_POST['element']!=""){
				...
				if (strlen($siteid)==7 && strlen($candidate)==8){
					$net1_date=get_net1_date($siteid,$candidate,$code,'','NEW',$conn_Netone);
					...
					$net1Out= "Please provide SITE ID (7 characters) and candidate (8 characters) or candidate and upgrade number";
					$type="error";
				}	
			}
			...
			$net1_date=get_net1_date($siteid,$candidate,$code,$_POST['upgnr'],'UPG',$conn_Netone);
			if ($net1_date!="ERROR" && $net1_date!="ERRORCAND"){
				$net1Out="<b>".$net1_date['SIT_UDK'].": ".$code."</b><br>COMPLETE: ".$net1_date['complete']."<br>ESTIMATE: ".$net1_date['estimate']."<br>NOTES: ".$net1_date['notes'];
				$type="info";
			}else if ($net1_date=="ERROR"){
				$net1Out="UPG: TASK NOT FOUND IN NET1 FOR THAT UPGRADE";
				$type="error";
			}else if ($net1_date=="ERRORCAND"){
				$net1Out="UPG: CANDIDATE IS NOT PREFERRED/FOUND";
				$type="error";
			}
		}else{
	...
	if (trim($data[0][0])!="SITE" or trim($data[0][1])!="UPGNR/CANDIDATE" or trim($data[0][2])!="CODE" or trim($data[0][3])!="DATE" or trim($data[0][4])!="ESTIMATE"){
		$res["type"]='error';
	...
	foreach ($data as $key => $result) {
		...
		if ($i!=0){
			...
			if ($result['0']=="" or strlen(trim($result['0']))!=7){
				$status='NOTOK';
				$general_status='NOTOK';
				$message.="SiteID must be 7 characters long<br>";
			}
			if (strlen(trim($result['1']))!=8 && substr(trim($result['1']),0,2)!="99"){
				$status='NOTOK';
				$general_status='NOTOK';
				$message.="Candidate must be 8 characters long<br>";
			}
			if ($result['3']!=''){
	...
	$data = array_map("str_getcsv", preg_split('/\r*\n+|\r+/', $_POST['csvdata']));
	if ($data[0][0]!="SITE" && $data[0][1]!="UPGNR/CANDIDATE" && $data[0][2]!="CODE" && $data[0][3]!="DATE"){
		$res["type"]='error';
		$res["message"]='Headers are incorrect';
		echo json_encode($res);
		exit;
	...
	foreach ($data as $key => $result) {
		if ($i!=0){
			...
			if (strlen(trim($result['1']))==8 && substr(trim($result['1']),0,2)!="99"){
				$candidate=trim($result['1']);
				$upgnr='';
			}else{
				$candidate='';
				$upgnr=trim($result['1']);
			}
			$out=updatecode(trim($result['0']),$candidate,$upgnr,trim($result['2']),$newdate,trim($result['4']),trim($result['5']),'yes',$conn_Netone,$filename);
	...
	foreach ($data as $key => $result){
		...
		if ($i!=0 && $result['0']!=''){
			...
			if ($result['1']!='IS' && $result['1']!='CL' && $result['1']!='DL' && $result['1']!='OH' && $result['1']!='ST' && $result['1']!='DM'){
				$status='NOTOK';
				$general_status='NOTOK';
				$message.="STATUS has to be IS, DL, CL, ST, DM or OH<br>";
			}
	...
	$out="<table class='table' style='width:700px;'><thead><th>ID</th><th>UPGNR/SITE</th><th>STATUS</th></thead>".$out."</table>";
	if ($general_status!='NOTOK'){
		$retval["type"]='info';
		$retval["table"]=$out;
	}else{
	...
	foreach ($data as $key => $result) {
		...
		if ($i!=0 && $result['0']!=''){
			...
			if ($result['1']!='WIP-A' && $result['1']!='WIP-C' && $result['1']!='CON' && $result['1']!='SAC' && $result['1']!='ALSAC'){
				$status='NOTOK';
				$general_status='NOTOK';
				$message.="CODE has to be WIP-A or WIP-C or CON or SAC or ALSAC<br>";
			}

			if (substr(trim($result['0']),0,2)=='99' || $result['1']=='WIP-A' || $result['1']=='SAC' || $result['1']=='ALSAC'){
				$query = "select WOR_UDK AS RES from WORKS_ORDERS@".$config['net1db']." WHERE WOR_UDK='".strtoupper(trim($result['0']))."'";
			...
			if ($result['2']=='' or ($result['2']!='TECHM' && $result['2']!='KPNGB' && $result['2']!='BENCHMARK'&& $result['2']!='M4C' && $result['2']!='ZTE')){
				$status='NOTOK';
				$general_status='NOTOK';
				$message.="Partner has to be TECHM, KPNGB, BENCHMARK, M4C or ZTE<br>";
	...
	foreach ($data as $key => $result) {
		if ($i!=0 && $result['0']!=''){
			//CODES CAN BE FOUND IN PARTIES TABLE IN NET1
	...
	$data["table"]=$table;	
	echo json_encode($data);
}else if ($_POST['action']=="multianalyseRFinfo"){
	$data = array_map("str_getcsv", preg_split('/\r*\n+|\r+/', $_POST['csvdataStatus']));

	if ($data[0][0]!="SITE_UPGNR" && $data[0][1]!="RFINFO"){
		$res["type"]='error';
		$res["message"]='Headers are incorrect';
		echo json_encode($res);
		exit;
	...
	$out="<table class='table' style='width:700px;'><thead><th>ID</th><th>UPGNR/SITE</th><th>RFINFO</th></thead>".$out."</table>";
	if ($general_status!='NOTOK'){
		$retval["type"]='info';
		$retval["table"]=$out;
	}else{

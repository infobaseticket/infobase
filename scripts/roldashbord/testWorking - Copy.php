...
for ($i = 0; $i < count($res1['TOTAL']); $i++){
	...
	if ($res1['YEAR'][$i]!=$prev_year){
		...
		if($i!=0){
			$dataArrays.="{
		            title : {'text':'".$res1['YEAR'][$i]." DISMANTLED SITES per month'},
		            series : [
		                {'data': dataMap.dataDISM['".$res1['YEAR'][$i]."']}
		            ]
		        },";
	    }
	 
	    $yearsArray[$z]=$res1['YEAR'][$i];
	    $years.="'".$res1['YEAR'][$i]."',";
	...
	$data[$res1['MONTH'][$i]][$res1['YEAR'][$i]]=$res1['TOTAL'][$i];
	
}
$years=substr($years, 0,-1);
$dataArrays=substr($dataArrays, 0,-1);
//echo $dataArrays;
//echo "<pre>".print_r($yearsArray,true);
...
/*
dataMap.dataDISM = dataFormatter({
    <?=$outDISM?>
});

dataMap.dataDISM2 = dataFormatter({
    <?=$outDISM?>
});
*/

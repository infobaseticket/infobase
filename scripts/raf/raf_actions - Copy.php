...
if ($_POST['action']=="insert_new_raf"){
	...
	if ($message){
		...
		if ($_POST['rafid']){
			...
			if ($_POST['type']=="MOV Upgrade" or $_POST['type']=="TECHNO Upgrade" or $_POST['type']=="MOD Upgrade" or substr_count($_POST['type'], 'New')=="1" ){
	...
	*/
			...
			$query = "INSERT INTO INFOBASE.BSDS_RAFV2 (
			   ...
			   NET1_FUND,			   
			   PARTNER_RFPACK, PARTNER_RFPACK_DATE, PARTNER_RFPACK_BY,
		...
		if (!$stmt) {
			...
			if (($_POST['region']=="MT" && $_POST['rafid']=="") or $_POST['type']=='New All Areas'){ //=> also for all areas
				$query = "INSERT INTO BSDS_RAF_PO (RAFID,CONFIRMED,CONFIRMED_PARTNER,ACQCON,INSERTDATE) VALUES ('".$res6['RAFID'][0]."','CONFIRMED','CONFIRMED','ACQ',SYSDATE)";
				$stmt = parse_exec_free($conn_Infobase, $query, $error_str);
				if (!$stmt) {
					die_silently($conn_Infobase, $error_str);
				}else{
				...
				$query = "INSERT INTO BSDS_RAF_PO (RAFID,POPR,CONFIRMED,CONFIRMED_PARTNER,ACQCON,INSERTDATE) VALUES ('".$res6['RAFID'][0]."','NA','CONFIRMED','CONFIRMED','CON',SYSDATE)";
				$stmt = parse_exec_free($conn_Infobase, $query, $error_str);
				if (!$stmt) {
					die_silently($conn_Infobase, $error_str);
				}else{
...
if ($_POST['action']=="attach_BSDSKEY"){
	...
	if ($_POST['field']!='NET1_PAC_REJECT' && $_POST['field']!='NET1_FAC_REJECT'){
		...
		if ($_POST['field']=="RF_PAC_REJECT" || $_POST['field']=="BCS_TX_INP_REJECT" || $_POST['field']=="BCS_RF_INP_REJECT" || $_POST['field']=="BCS_NET1_STOP"
			|| $_POST['field']=="NET1_LINK_REJECT" || $_POST['field']=="NET1_FUND_REJECT" || $_POST['field']=="PARTNER_ACQUIRED_REJECT"
		    || $_POST['field']=="TXMN_ACQUIRED_REJECT" || $_POST['field']=="COF_ACQ_REJECT" || $_POST['field']=="COF_CON_REJECT"
			|| $_POST['field']=="OTHER_INP"){
				if($config['mail']==true){
					if ($_POST['field']=="NET1_LINK_REJECT"){
						$region=substr($_POST['siteid'],0,2);
						$fullname="RADIO ".$region." REGION";
						$email= "Radio.Optimisation.".$region."@basecompany.be";
					}else if($_POST['field']=="OTHER_INP"){
						$user_BY=getuserdata($_POST['creator']);
						$fullname=$user_BY['fullname'];
						$email=$user_BY['email'];
					}else{
						$user_BY=getuserdata($_POST['previous_user']);
						$fullname=$user_BY['fullname'];
						$email=$user_BY['email'];
					}
					...
					$mail->Subject    = $_POST['siteid']." RAF ".$_POST['id']." rejected";
					$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test
					$text_body = "Hi ".$fullname.",<br>";
				   	$text_body .= "The RAF with ID ".$_POST['id']." has been REJECTED for ".$_POST['siteid']."!<br><br>";
				   	$text_body .= "This has been done by <a href='mailto:'".$email_sender."'>".$fullname_sender."</a><br>";
				   	$text_body .= "<u>Reason:</u><br>".$_POST['value']."<br>";
				   	$text_body .= "http://infobase/bsds/index.php?module=RAF&rafid=".$_POST['id']."<br><br>";
			...
			$res["success"]=true;
			echo json_encode($res);
		}else if ($config['mail']==true && $_POST['field']=="PARTNER_RFPACK" && $_POST['raftype']=="New Indoor" && $val=="OK"){
					...
					$email_sender=$userdetails_Sender['email'];
					$mail->SetFrom($email_sender, 'Infobase');
					$mail->AddReplyTo("freeyland@gmail.com","Frederick Eyland");
					$mail->Subject    = $_POST['siteid']." RAF ".$_POST['id']." PARTNER PAC";
					$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test
					$text_body = "Hi ".$fullname.",<br>";
				   	$text_body .= "The RAF with ID ".$_POST['id']." has been approved by PARTNER!<br><br>";
				   	$text_body .= "This has been done by <a href='mailto:'".$email_sender."'>".$fullname_sender."</a><br>";
				   	$text_body .= "http://infobase/bsds/index.php?module=RAF&rafid=".$_POST['id']."<br><br>";
				   	$text_body .= "<br><br>For Citrix users: please start Infobase via citrix and copy this link in the Infobase window.<br><br>";
				   	$text_body .= "Rgds,<br>From Frederick Eyland for Infobase<br><br>";
					...
					$mail->AddAddress('Bruno.Beckers@basecompany.be', 'Bruno Beckers');
	...
	}else if($_POST['field']=='NET1_PAC_REJECT' or $_POST['field']=='NET1_FAC_REJECT'){
		$query3="INSERT INTO INFOBASE.BSDS_RAF_PACFACHIST (RAFID, ACTION_DATE, STATUS,ACTION_BY,FIELD) VALUES ('".$_POST['id']."',SYSDATE,'".escape_sq($_POST['value'])."','".$guard_username."','".$_POST['field']."')";
		//echo $query3;
		$stmt3 = parse_exec_free($conn_Infobase, $query3, $error_str);
		if (!$stmt3) {
			die_silently($conn_Infobase, $error_str);
...
*/	
	...
	if ($_POST['PIF']==1 && $_POST['STAB']==1 && $_POST['OTHER']==1){
		...
		$query4 = "UPDATE BSDS_RAFV2 SET NET1_PAC='NOT OK',NET1_FAC='NOT OK',PARTNER_RFPACK='NOT OK' WHERE RAFID='".$_POST['rafid']."'";	
		//echo $query4;
		$stmt = parse_exec_free($conn_Infobase, $query4, $error_str);
		if (!$stmt) {
			die_silently($conn_Infobase, $error_str);

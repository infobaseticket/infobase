...
require_once($config['sitepath_abs']."/bsds/PHPlibs/phpExcel/PHPExcel/IOFactory.php");
...
$objPHPExcel->getProperties()->setCreator("Infobase")
							 ...
							 ->setDescription("RAF report generated by Infobase on selected filters.")
...
$objDrawing->setPath($config['sitepath_abs']."/infobase/files/business-fever/img/basecompany.png");
...
$objDrawing->setName('PHPExcel logo');
$objDrawing->setDescription('KPNGroup logo');
$objDrawing->setPath($config['sitepath_abs']."/infobase/files/business-fever/img/business-fever/img/logo.png");
...
$objPHPExcel->setActiveSheetIndex(0)
	...
	->setCellValue('AD3', 'Vendor G9')
	->setCellValue('AE3', 'Vendor G18')
	->setCellValue('AF3', 'Vendor 3G')
	->setCellValue('AG3', 'Vendor L18')
	->setCellValue('AH3', 'Vendor L26')
	->setCellValue('AI3', 'BAND G9')
	->setCellValue('AJ3', 'BAND G18')
	->setCellValue('AK3', 'BAND U21')
	->setCellValue('AL3', 'BAND U9')
	->setCellValue('AM3', 'BAND L18')
	->setCellValue('AN3', 'BAND L26')
	->setCellValue('AO3', 'COMMERCIAL PHASE');;
...
if ($amount_of_RAFS>=1){
	...
	for ($i = 0; $i <$amount_of_RAFS; $i++) {
		...
		if (!$stmt2) {
			...
			if($res2['BAND_LTE1800'][0]==1){
				$band.="LTE1800 ";
				$vendor_4G_LTE1800=$res2['VENDOR4G_LTE1800'][0];
			}
			if($res2['BAND_LTE2600'][0]==1){
				$band.="LTE2600 ";
				$vendor_4G_LTE2600=$res2['VENDOR4G_LTE2600'][0];
			}
		}
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("I".$row, $res1['PARTNER_INP'][$i])
					->setCellValue("J".$row, $res1['BCS_NET1'][$i])
					->setCellValue("K".$row, $res1['NET1_LBP'][$i])
					->setCellValue("L".$row, $res1['PARTNER_ACQUIRED'][$i])
					->setCellValue("M".$row, $res1['TXMN_ACQUIRED'][$i])
					->setCellValue("N".$row, $res1['NET1_ACQUIRED'][$i])
					->setCellValue("O".$row, $res1['RADIO_FUND'][$i])
					->setCellValue("P".$row, $res1['UA503'][$i])
					->setCellValue("Q".$row, $res1['NET1_FUND'][$i])
					->setCellValue("R".$row, $res1['PARTNER_PAC'][$i])
					...
					->setCellValue("AG".$row, $vendor_4G_LTE1800)
					->setCellValue("AH".$row, $vendor_4G_LTE2600)
					->setCellValue("AI".$row, $res2['BAND_900'][0])
					->setCellValue("AJ".$row, $res2['BAND_1800'][0])
					->setCellValue("AK".$row, $res2['BAND_UMTS'][0])
					->setCellValue("AL".$row, $res2['BAND_UMTS900'][0])
					->setCellValue("AM".$row, $res2['BAND_LTE1800'][0])
					->setCellValue("AN".$row, $res2['BAND_LTE2600'][0])
					->setCellValue("AO".$row, $res2['COMMERICAL'][0]);
